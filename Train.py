...
def main():
    ...
    n_grams = [return_best_n_grams(n, args.top_ngrams, total)
               for n in [1, 2, 3, 4]]

    print('------------', '\n', 'Generating POS n-grams...')

    pos_n_grams = [return_best_n_grams(
    ...
    for index, row in data.iterrows():
        ...
        X.append(ngram_rep(row['text'], row['POS'], [
                 n_grams, pos_n_grams, word_n_grams]))
    ...
    training_set = torch.utils.data.DataLoader(
        training_Loader, batch_size=args.batch_size, shuffle=True)
    validation_set = torch.utils.data.DataLoader(
        validation_Loader, batch_size=args.batch_size, shuffle=False)
